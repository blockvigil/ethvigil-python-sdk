...
def generate_contract_function(**outer_kwargs):
    def fn(self, *params_args, **params_kwargs):
        ...
        ev_core_logger.debug('Calling EthVigil contract function')
...
class EVCore(object):
        ...
        try:
            ...
            if try_login:
                try:
                    ...
                    if self._verbose:
                        ev_core_logger.info('Could not connect to EthVigil endpoint. Attempting to load account information from cache.')
                    try:
                        with open(pwd.getpwuid(os.getuid()).pw_dir + '/.ethvigil/account_info.json', 'r') as f:
                    ...
                    if self._verbose:
                        for k in r:
                            ...
                            if k == 'contracts':
                                ...
                                ev_core_logger.info(f'EthVigil API key: \t {d}\n=============\n')
                            elif k == 'api_prefix':
                    ...
                    with open(pwd.getpwuid(os.getuid()).pw_dir + '/.ethvigil/account_info.json', 'w') as f:
    ...
    #     try:
    #         os.stat(pwd.getpwuid(os.getuid()).pw_dir + '/.ethvigil/contracts')
    #     except:
    #         os.mkdir(pwd.getpwuid(os.getuid()).pw_dir + '/.ethvigil/contracts')
    #     os.mkdir(pwd.getpwuid(os.getuid()).pw_dir + f'/.ethvigil/contracts/{app_name}')
    #     z.extractall(pwd.getpwuid(os.getuid()).pw_dir + f'/.ethvigil/contracts/{app_name}')
    #     sys.path.append(pwd.getpwuid(os.getuid()).pw_dir + f'/.ethvigil/contracts/{app_name}')
    ...
    def generate_contract_sdk(self, contract_address, app_name):
        ...
        # return pwd.getpwuid(os.getuid()).pw_dir + f'/.ethvigil/contracts/{app_name}'
    ...
    def signup(self, invite_code):
        ...
        ev_core_logger.debug('Attempting to signup with EthVigil')
    ...
    def deploy(self, contract_file, contract_name, inputs):
        ...
        if self._verbose:
            ev_core_logger.debug('EthVigil deploy response: ')
